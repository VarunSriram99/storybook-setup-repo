  ...
  return (
    <div
      className={classnames({
        "p-6": padded,
        "cs-ui-bg-gray-100": style === ACCORDION_STYLES.secondary,
        [className]: className,
      })}
    >
      {React.Children.map(children, (child, index) => {
        ...
        return React.cloneElement(child, {
          ...
          className: classnames(child.props.className, {
            "cs-ui-accordion__wrapper--last-item": isSingleOrLastChild,
          }),
          onClick: () => {
            if (openTab === index) {
              setOpenTab(null);
...
export const Item = ({
  ...
  return (
    <div
      className={classnames("cs-ui-accordion__wrapper", {
        [className]: className,
      })}
    >
      <div
        ...
        role="button"
        onClick={onClick}
        id={`cs-ui-accordion-item-${id}`}
        aria-disabled={isOpen}
        aria-expanded={isOpen}
        aria-controls={`cs-ui-accordion-section-${id}`}
      ...
      >
        <div
          {...titleProps}
          className={classnames(
            "cs-ui-accordion__item-handle flex-grow-1 flex items-center",
            {
              "cs-ui-text-gray-600": !isOpen,
              "cs-ui-text-gray-800": isOpen,
            }
          )}
        >
          {title}
        </div>
        {
          <motion.div
            ...
            className="cs-ui-accordion__item-toggle-icon flex-grow-0"
          >
            <RightMinimalArrow size={16} color="#68737D" {...iconProps} />
          </motion.div>
        }
      </div>
      ...
      <Collapse
        role="region"
        open={isOpen}
        className="cs-ui-accordion__drop"
        id={`cs-ui-accordion-section-${id}`}
        aria-labelledby={`cs-ui-accordion-item-${id}`}
      >
        {children}
      </Collapse>
    </div>
